...
class PlayerDB:
    ...
    async def get_players(self, event_id):
        async with grpc.aio.insecure_channel(environ['EVENT_DB_ENDPOINT']) as channel:
            ...
            response = await stub.GetEventPlayers(proto.hermes_pb2.GetEventPlayersRequest(event_id=event_id))
    ...
    async def get_magic_token(self, player_dc_id):
        async with grpc.aio.insecure_channel(environ['EVENT_DB_ENDPOINT']) as channel:
            stub = proto.hermes_pb2_grpc.EventDbStub(channel)
            response = await stub.GetMagicToken(proto.hermes_pb2.GetMagicTokenRequest(player_dc_id=str(player_dc_id)))
    ...
    async def sync_event_players(self, event_players):
        ...
        for event_player in event_players:
            response_list.append(proto.hermes_pb2.SyncEventPlayerRequestEventPlayers(**event_player))

        async with grpc.aio.insecure_channel(environ['EVENT_DB_ENDPOINT']) as channel:
            stub = proto.hermes_pb2_grpc.EventDbStub(channel)
            await stub.SyncEventPlayers(proto.hermes_pb2.SyncEventPlayerRequest(event_players=response_list))
